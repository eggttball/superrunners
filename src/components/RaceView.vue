      ...
      <div class="lane-cards">
        <button v-for="(athlete, index) in athletes" :key="athlete.student.id" class="lane-card" @click="game.studentId = athlete.student.id" :style="{ '--runner-color': currentClass.color }">
          ...
          <span class="runner-card-portrait">
            <PixelRunner :seed="athlete.student.id" :gender="athlete.student.gender" :height="athlete.student.height" :weight="athlete.student.weight" :speed="speedAt(Math.max(0, elapsed), athlete.profile)" :color="currentClass.color" :size="47" :running="phase === 'running' && !paused && speedAt(Math.max(0, elapsed), athlete.profile) > 0" />
          </span>
