...
function runnerTransform(runner) {
  ...
  const size = props.raceMode ? 3.1 : 2.6
...
<template>
  <div class="track-viewport" :class="{ 'is-race': raceMode }">
    <svg
      class="track-svg"
      :viewBox="raceMode ? sprintViewBox : TRACK.viewBox"
      role="img"
    ...
    >
      <defs>
        <linearGradient :id="surfaceId" x1="0" y1="0" x2="0" y2="1">
          ...
          <stop offset="100%" stop-color="#be7053" />
        </linearGradient>
        <linearGradient :id="grassId" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stop-color="#32634b" />
          <stop offset="100%" stop-color="#264e3d" />
        </linearGradient>
        <clipPath :id="infieldId"><path :d="ovalPath(TRACK.innerRadius)" /></clipPath>
      </defs>

      <rect x="-65" y="-65" width="215" height="132" fill="#dce5d1" />
      <path :d="ovalPath(TRACK.outerRadius + 5)" fill="none" stroke="#d0d7c4" stroke-width="2.3" />
      <path :d="ovalPath(TRACK.outerRadius + 2.2)" fill="#c5cfbc" />
      ...
      <g v-for="(tree, index) in trees" :key="index" :transform="`translate(${tree[0]} ${tree[1]}) scale(${tree[2]})`" shape-rendering="crispEdges">
        ...
        <path d="M-2-3h4v7h-4zM-4-1h8v3h-8zM-3-2h6v5h-6z" fill="#42634a" />
        <path d="M-2-3h3v5h-3zM-3-1h5v2h-5z" fill="#547754" />
        <path d="M-2-2h2v2h-2z" fill="#6f8a5e" />
      </g>
      ...
      <g :clip-path="`url(#${infieldId})`">
        <rect v-for="stripe in stripes" :key="stripe" :x="-40 + stripe * 10" y="-37" width="5" height="74" fill="#a1bf87" opacity=".04" />
      </g>
      ...
      <text :x="TRACK.sprintStartX + 50" :y="TRACK.outerRadius + 9" class="sprint-caption">100 M · 每一步，都更靠近自己</text>
      ...
      <g v-for="runner in displayedRunners" :key="runner.id" :transform="runnerTransform(runner)" :data-lane="runner.lane" :data-distance="runner.distance" :opacity="runner.opacity ?? 1">
        ...
        <PixelRunner :seed="String(runner.id)" :gender="runner.gender" :height="runner.height" :weight="runner.weight" :speed="runner.speed" :facing="runner.facing" :color="runner.color || '#e3ee94'" :running="runner.moving" :size="raceMode ? 3.1 : 2.6" :backdrop="false" />
      </g>
    </svg>
  </div>
</template>
...
.sprint-caption { font-family: sans-serif; font-size: 1.8px; font-weight: 700; letter-spacing: .32px; text-anchor: middle; fill: #4e6749; }
